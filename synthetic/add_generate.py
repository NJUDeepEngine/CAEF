...
from data.generator import AddSeqGenerator
from data.proportion import Proportioner
from turing_machine.alignment.aligner import TMAligner

train_target_file_template = 'datasets/train/{prefix}add{suffix}.json'
test_target_file_template = 'datasets/test/{prefix}add_{min}_{max}{suffix}.jsonl'
raw_train_target_file = 'datasets/raw/addition/train.jsonl'
raw_test_target_file_template = 'datasets/raw/addition/test_{min}_{max}.jsonl'

ADDITION_PROMPT = """The following is a state paired with a command to be executed of a Turing Machine that performs addition.
...
- There are three states in the machine: q0, q1, and qH. The machine starts in state q0 and halts when it reaches state qH. q1 is the state where the machine does the addition and calculates the carry out.
...
The machine performs addition by reading the digits from the two operands and writing the sum to the output tape. 
...
def seq_2_samples(seq, args):
    ...
    for i in range(len(seq) - 1):
        input = '' if args.no_prompt else ADDITION_PROMPT
        input += seq[i][0] + '\n' + seq[i][1] + '\n'
        output = seq[i + 1][0] + '\n' + seq[i + 1][1] + '\n'
